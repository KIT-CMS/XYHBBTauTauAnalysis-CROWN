...
def add_ak4jet_config(configuration: Configuration, era: str, profile):
    ...
    configuration.add_config_parameters(
        "global",
        {
            # JME publishes jet ID files for Run 3 only. The 2018 UL NanoAOD-v15
            # file is generated by the jetid-payloads repository (provenance and
            # checksum in payloads/jetid/README.md) and used on the SM 2018-v15
            # path; NMSSM 2018 reads the v9 Jet_jetId branch and needs no file.
            "ak4jet_id_file": EraModifier(
                {
                    ...
                    **(
                        {"2018": "payloads/jetid/Run2-2018-UL-NanoAODv15/jetid.json.gz"}
                        if use_sm_2018_v15
                        else {}
                    ),
                    **{
...
def build_config(
    ...
    # first member of the auxiliary `Jet` quantity group below. The SM 2018-v15
    # AK4-PUPPI path substitutes that member (two producers defining `Jet_ID`
    # would only surface as an RDataFrame redefinition at run time): NanoAOD v15
    # drops the precomputed Jet_jetId branch that the legacy v9 rename producer
    # reads, so the ID is evaluated from the composition branches with the
    # correctionlib producer that 2024/2025 already use, pointed at the 2018 UL
    # payload via `ak4jet_id_file`. NMSSM (use_2018_v15_jet_path=False) keeps
    # the v9 rename producer.
    jet_id_overrides = {}
    if profile.use_2018_v15_jet_path and era == "2018":
        jet_id_overrides["2018"] = jets.JetIDFromCorrectionlib

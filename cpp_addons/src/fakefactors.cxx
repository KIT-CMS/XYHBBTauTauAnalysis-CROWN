...
ROOT::RDF::RNode RawFakeFactorSemileptonic(
  ...
  auto raw_ff_semileptonic = [logger_name, qcd_cset, tt_cset, fractions_cset,
    ...
    Logger::get(logger_name)->debug("    {}: {}", ff_tt_name, ff_tt_variation);
    Logger::get(logger_name)
        ->debug("    {}: {}", ff_fraction_name, ff_fraction_variation);

    // Prepare the inputs for the correction set evaluation
    auto _qcd_inputs = fakefactors::util::prepare_ff_input(qcd_inputs, ff_qcd_variation);
    auto _tt_inputs = fakefactors::util::prepare_ff_input(tt_inputs, ff_tt_variation);
    auto _fraction_inputs_qcd =
        fakefactors::util::prepare_fractions_input(fraction_inputs, "QCD", ff_fraction_variation);
    auto _fraction_inputs_tt = fakefactors::util::prepare_fractions_input(fraction_inputs, "ttbar",
                                                       ff_fraction_variation);
    ...
    Logger::get(logger_name)
        ->debug("    cset {}: {}", ff_qcd_name, fakefactors::util::join(_qcd_inputs));
    Logger::get(logger_name)
        ->debug("    cset {}: {}", ff_tt_name, fakefactors::util::join(_tt_inputs));
    Logger::get(logger_name)
...
ROOT::RDF::RNode FakeFactorSemileptonic(
  ...
  auto tt_corr_closure_cset =
      correctionManager.loadCompoundCorrection(corr_file, corr_tt_closure_name);
  ...
  auto ff_semileptonic = [qcd_cset, tt_cset, fractions_cset,
                          ...
                          tt_corr_closure_variation](
                             ...
                             const std::vector<float> &qcd_corr_closure_inputs,
    ...
    Logger::get(logger_name)
        ->debug("    {}: {}", ff_fraction_name, ff_fraction_variation);
    Logger::get(logger_name)
        ->debug("    {}: {}", corr_qcd_dr_sr_name, qcd_corr_dr_sr_variation);
    Logger::get(logger_name)
        ->debug("    {}: {}", corr_qcd_closure_name,
                qcd_corr_closure_variation);
    Logger::get(logger_name)
        ->debug("    {}: {}", corr_tt_closure_name, tt_corr_closure_variation);
    ...
    auto _fraction_inputs_tt = fakefactors::util::prepare_fractions_input(fraction_inputs, "ttbar",
                                                       ff_fraction_variation);
    auto _qcd_corr_dr_sr_inputs =
        fakefactors::util::prepare_ff_input(qcd_corr_dr_sr_inputs, qcd_corr_dr_sr_variation);
    auto _qcd_corr_closure_inputs =
        fakefactors::util::prepare_ff_input(qcd_corr_closure_inputs, qcd_corr_closure_variation);
    auto _tt_corr_closure_inputs =
        fakefactors::util::prepare_ff_input(tt_corr_closure_inputs, tt_corr_closure_variation);
    ...
    // Calculate the raw fake factor
    float ff =
        frac_qcd * std::max(ff_qcd * corr_qcd_dr_sr * corr_qcd_closure, 0.f) +
...
ROOT::RDF::RNode raw_fakefactor_lt(
  ...
  auto df1 =
      df.Define(outputname, calc_fake_factor, {tau_pt, njets, lep_mt, nbtags});
  return df1;
}
...
ROOT::RDF::RNode raw_fakefactor_nmssm_tt(
  ...
  auto fractions =
      correction::CorrectionSet::from_file(ff_file)->at("process_fractions");
  auto fractions_subleading = correction::CorrectionSet::from_file(ff_file)->at(
  ...
  auto calc_fake_factor = [tau_idx, qcd_variation, ttbar_variation,
    ...
    if (pt_2 >= 0.) {
      ...
      if (tau_idx == 0) {
        ...
        Logger::get("RawFakeFactor")->debug("QCD - value {}", qcd_ff);
        ttbar_ff =
            ttbar_subleading->evaluate({pt_1, (float)njets, ttbar_variation});
        Logger::get("RawFakeFactor")->debug("ttbar - value {}", ttbar_ff);
...
ROOT::RDF::RNode fakefactor_nmssm_lt(
  ...
  Logger::get("FakeFactor")
      ->debug("Fraction variation - Name {}", fraction_variation);
  Logger::get("FakeFactor")
      ->debug("QCD lep pt corr variation - Name {}", qcd_corr_leppt_variation);
  Logger::get("FakeFactor")
  ...
  Logger::get("FakeFactor")
      ->debug("Wjets DRSR corr variation - Name {}", wjets_corr_drsr_variation);
  Logger::get("FakeFactor")
  ...
  auto calc_fake_factor = [qcd_variation, wjets_variation, ttbar_variation,
                           ...
                           qcd_corr_taumass_variation, qcd_corr_drsr_variation,
                           wjets_corr_leppt_variation,
                           wjets_corr_taumass_variation,
                           wjets_corr_drsr_variation,
                           ttbar_corr_leppt_variation,
                           ttbar_corr_taumass_variation, qcd, wjets, ttbar,
                           fractions, qcd_lep_pt_closure, qcd_tau_mass_closure,
                           qcd_DR_SR, wjets_lep_pt_closure,
                           wjets_tau_mass_closure, wjets_DR_SR,
                           ttbar_lep_pt_closure, ttbar_tau_mass_closure](
    ...
    if (pt_2 >= 0.) {
      ...
      ff = qcd_frac * std::max(qcd_ff, (float)0.) * qcd_lep_pt_corr *
           ...
           wjets_frac * std::max(wjets_ff, (float)0.) * wjets_lep_pt_corr *
               wjets_tau_mass_corr * wjets_DR_SR_corr +
...
ROOT::RDF::RNode fakefactor_nmssm_boosted_lt(
  ...
  Logger::get("FakeFactor")
      ->debug("QCD lep pt corr variation - Name {}", qcd_corr_leppt_variation);
  Logger::get("FakeFactor")
      ->debug("QCD lep mt corr variation - Name {}", qcd_corr_lepmt_variation);
  Logger::get("FakeFactor")
      ->debug("QCD DRSR corr variation - Name {}", qcd_corr_drsr_variation);
  Logger::get("FakeFactor")
      ->debug("Wjets lep pt corr variation - Name {}",
  ...
  auto calc_fake_factor =
      [qcd_variation, wjets_variation, ttbar_variation, fraction_variation,
       qcd_corr_leppt_variation, qcd_corr_lepmt_variation,
       qcd_corr_drsr_variation, wjets_corr_leppt_variation,
       wjets_corr_drsr_variation, ttbar_corr_leppt_variation, qcd, wjets, ttbar,
       fractions, qcd_lep_pt_closure, qcd_lep_mt_closure, qcd_DR_SR,
...
ROOT::RDF::RNode fakefactor_nmssm_tt(
  ...
  auto calc_fake_factor = [tau_idx, qcd_variation, ttbar_variation,
                           ...
                           qcd_corr_taumass_variation, qcd_corr_drsr_variation,
                           ttbar_corr_leppt_variation,
                           ttbar_corr_taumass_variation, qcd, ttbar, fractions,
                           qcd_tau_pt_closure, qcd_tau_mass_closure, qcd_DR_SR,
                           ttbar_tau_pt_closure, ttbar_tau_mass_closure,
                           qcd_subleading, ttbar_subleading,
                           fractions_subleading, qcd_tau_pt_closure_subleading,
    ...
    if (pt_2 >= 0.) {
      ...
      if (tau_idx == 0) {
        ...
        ff = (qcd_frac + wjets_frac) * std::max(qcd_ff, (float)0.) *
             ...
             ttbar_frac * std::max(ttbar_ff, (float)0.) * ttbar_tau_pt_corr *
                 ttbar_tau_mass_corr;
      } else if (tau_idx == 1) {
        qcd_ff = qcd_subleading->evaluate({pt_2, (float)njets, qcd_variation});
        Logger::get("FakeFactor")->debug("QCD - value {}", qcd_ff);
        ttbar_ff =
            ttbar_subleading->evaluate({pt_2, (float)njets, ttbar_variation});
        Logger::get("FakeFactor")->debug("ttbar - value {}", ttbar_ff);
        qcd_frac = fractions_subleading->evaluate(
            {"QCD", m_vis, (float)nbtag, fraction_variation});
        Logger::get("FakeFactor")->debug("QCD - fraction {}", qcd_frac);
        ...
        Logger::get("FakeFactor")
            ->debug("QCD - lep mass correction {}", qcd_tau_mass_corr);
        qcd_DR_SR_corr =
            qcd_DR_SR_subleading->evaluate({m_vis, qcd_corr_drsr_variation});
        Logger::get("FakeFactor")
            ->debug("QCD - DR to SR correction {}", qcd_DR_SR_corr);
        ...
        Logger::get("FakeFactor")
            ->debug("ttbar - lep pt correction {}", ttbar_tau_pt_corr);
        ttbar_tau_mass_corr = ttbar_tau_mass_closure_subleading->evaluate(
            {mass_2, ttbar_corr_taumass_variation});
        Logger::get("FakeFactor")
            ->debug("ttbar - lep mass correction {}", ttbar_tau_mass_corr);

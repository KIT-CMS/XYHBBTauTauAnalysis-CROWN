 ...
 *
 * @note It is advisable to use the `sf_dependence` mode `"dm"` for Run 3
 * analyses. This is also recommended by the TAU POG.  For each decay mode, the
 * scale factor is parametereized as a separate function of the tau \f$p_T\f$.
 * The `sf_dependence` mode `"pt"` should be used with care. The use of this set
 * of scale factors has not been tested thoroughly in this framework.
 *
 * The parameter `variation` can take strings consistent with the following
 * expressions:
 *
 * - `nom`: The nominal scale factor.
 * - `stat$i_dm$DM_$updown`: Statistical uncertainties on fit parameters,
 * uncorrelated across DMs and eras.
 * - `syst_allears_$updown`: Systematic uncertainty component correlated across
 * all DMs and eras.
 * - `syst_$era_$updown`: Systematic uncertainty component correlated across DMs
 * and uncorrelated across eras.
 * - `syst_TES_$era_dm$DM_$updown`: Systematic uncertainty due to uncertainty in
 * TES, uncorrelated across eras and decay modes.
 *
 * For the `$` expressions, substitute, `$i = 1, 2`, `$DM = 0, 1, 10, 11`,
 * `$era = 2022_preEE, 2022_postEE, 2023_preBPix, 2023_postBPix`, `$updown = up,
 * down`.
 *
 * @warning Since the latest update the general CROWN function can be used again
 ...
 * @param sf_dependence variable dependence of the scale factor, opions are "pt"
 * or "dm"
 * @param variation name of the scale factor variation. The different
 * possibilities to use  for this this parameter are listed above.
 *

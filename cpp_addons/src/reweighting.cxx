...
/**
 * @brief Function used to evaluate the lheScaleweight of an event from NMSSM
signal samples. The weights are stored in the nanoAOD file an defined as
w_var/w_nominal. Depending on the selected muR and muF value, a specific index
has to be returned. The mapping between the index and the muR and muF values is:
...
In the NMSSM sample is now state for muF=1, muR=1 saved, which is different to
other nanoAOD samples.

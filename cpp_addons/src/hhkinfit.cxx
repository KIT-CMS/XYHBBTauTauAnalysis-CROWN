...
YHKinFit(ROOT::RDF::RNode df, const std::string &outputname_1,
  ...
  auto kin_fit = [YDecay](const float &tau_pt_1, const float &tau_eta_1,
    ...
    if ((tau_pt_1 > 0.) && (tau_pt_2 > 0.) && (b_pt_1 > 0.) && (b_pt_2 > 0.)) {
      ...
      ROOT::Math::PtEtaPhiEVector met_LV =
          (ROOT::Math::PtEtaPhiEVector)ROOT::Math::PtEtaPhiMVector(met, 0.,
                                                                   met_phi, 0.);
      ...
      std::vector<int> hypo_mY = {
          50,   60,   70,   80,   90,   95,   100,  125,  150,  250,  300,
          400,  500,  600,  700,  800,  900,  1000, 1100, 1200, 1300, 1400,
          1600, 1800, 2000, 2200, 2400, 2500, 2600, 2800, 3000, 3500};
      ...
      YHKinFitMaster kinFits =
          YHKinFitMaster(b_1, b_reso_1, b_2, b_reso_2, tau_1, tau_2, met_LV,
                         met_cov, YToTauTau);
      ...
      Logger::get("YHKinFit" + YDecay)->debug("kinfit_chi2: {}", kinfit_chi2);
      Logger::get("YHKinFit" + YDecay)->debug("kinfit_prob: {}", kinfit_prob);
    }

    ROOT::RVec<float> result = {(float)kinfit_convergence, (float)kinfit_mX,
  ...
  auto df1 = df.Define(result_vec_name, kin_fit,
                       {tau_pt_1,  tau_eta_1, tau_phi_1,  tau_mass_1, tau_pt_2,
                        tau_eta_2, tau_phi_2, tau_mass_2, b_pt_1,     b_eta_1,
                        b_phi_1,   b_mass_1,  b_reso_1,   b_pt_2,     b_eta_2,
                        b_phi_2,   b_mass_2,  b_reso_2,   met,        met_phi,

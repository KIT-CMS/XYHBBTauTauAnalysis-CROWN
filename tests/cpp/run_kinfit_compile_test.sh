...
# its own location. Needs ROOT + g++ and the spdlog headers (e.g. the
# kingmaker_standalone container). Point it at a non-default toolchain with:
#
...
OUT_BIN="${TMPDIR:-/tmp}/test_sm_hh_kinfit"
...
if [[ -z "${SPDLOG_INCLUDE_DIR:-}" || ! -d "${SPDLOG_INCLUDE_DIR}/spdlog" ]]; then
    echo "[run_kinfit_compile_test] no spdlog headers found under" \

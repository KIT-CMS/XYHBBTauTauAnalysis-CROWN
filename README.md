...
- **Jet ID from correctionlib** -- v15 drops the precomputed `Jet_jetId`
  ...
  (`Jet_chHEF`, `Jet_neEmEF`, ...) with the same correctionlib producer that
  2024/2025 use, pointed at `payloads/jetid/Run2-2018-UL-NanoAODv15/jetid.json.gz`.
  JME does not publish that file for Run 2 UL; it is generated by the
  `jetid-payloads` repository (provenance and checksum in
  `payloads/jetid/README.md`). The 0/2/6 encoding and the `ak4jet_id_wp`
